...
import fr.velinfo.avro.record.stream.AvroNearbyTraffic;
...
public class DailyStationNearbyTrafficStreamBuilder {
    ...
    public KStream<String, AvroNearbyTraffic> build(KStream<String, AvroBicycleCount> stationNearbyCountsStream){
        ...
        var materialized = Materialized.<String, AvroNearbyTraffic, WindowStore<Bytes, byte[]>>with(Serdes.String(), StreamUtils.avroSerde())

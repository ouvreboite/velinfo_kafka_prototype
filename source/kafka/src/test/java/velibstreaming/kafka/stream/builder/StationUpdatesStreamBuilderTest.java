...
class StationUpdatesStreamBuilderTest {
    ...
    public void stationUpdate_shouldBeEmittedWithAStaleTimestamp_whenAvailabilityIsADuplicateForMoreThan15Minutes() {
        ...
        assertEquals(firstEmitted.getLoadTimestamp(),records.get(0).getValue().getStaleSinceTimestamp(), "When the same availability is re-emitted more than 15 minutes later, a new update should be emitted and its staleSinceTimestamp should use the loadTimestamp of the first occurrence");
